...
>
    ...
    <div class="flex flex-col items-start pr-8 w-full">
        <h6
            class="whitespace-pre-line text-lg font-medium mb-2 {TITLE_COLORS[
                mode
        ...
        {#if description}
            <p class="my-2 text-sm {TEXT_COLORS[mode]} {FONT_FAMILY_CLASS.secondary}">
                {description}
            </p>
        {/if}
        {#if labels.length}
            <labels-wrapper class="flex flex-wrap {TEXT_COLORS[mode]}">
                {#each labels as label}
                    <p
                        class="mt-2 mr-2 last:ml-0 py-1 px-3 border {TAG_BORDER_COLORS[
                            mode
